...
export function EmojiPopover({
  ...
  return (
    <>
      ...
      <div className="absolute left-0 top-full mt-1 z-50 w-64 max-w-[calc(100vw-2rem)] p-2 rounded-xl bg-(--app-surface) border border-(--app-line) shadow-xl grid grid-cols-8 gap-0.5">
        {COMPOSER_EMOJIS.map((e) => (
          <button
            key={e}
            type="button"
            onClick={() => onPick(e)}
            className="w-7 h-7 text-lg leading-none rounded hover:bg-(--app-surface-2) flex items-center justify-center"
          >
            {e}
          </button>
        ))}
