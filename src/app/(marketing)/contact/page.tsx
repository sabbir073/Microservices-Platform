...
const inp = "w-full rounded-xl bg-white/5 border border-white/10 px-4 py-3 text-white placeholder-slate-500 text-sm focus:outline-none focus:border-blue-500/40";
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 pt-16 pb-16 sm:pt-24">
      <div className="text-center">
        ...
        <p className="mt-4 text-slate-400 max-w-xl mx-auto">Questions, feedback, or need a hand? Our team is here 24/7 and typically replies within a few hours.</p>
      </div>
      ...
      <div className="mt-12 grid gap-6 lg:grid-cols-[1fr_1.4fr]">
        ...
        <div className="space-y-4">
          {[
            ...
            <a key={c.title} href={c.href ?? "#"} className={`flex items-start gap-3 rounded-2xl bg-white/5 border border-white/10 p-4 ${c.href ? "hover:border-blue-500/30 transition-all" : ""}`}>
              ...
              <div><p className="text-sm font-bold text-white">{c.title}</p><p className="text-sm text-slate-400">{c.body}</p></div>
            </a>
          ))}
        </div>
        ...
        <div className="rounded-2xl bg-white/5 border border-white/10 backdrop-blur-xl p-6 sm:p-8">
          {sent ? (
            <div className="text-center py-8">
              ...
              <h2 className="mt-4 text-2xl font-extrabold text-white">Message sent</h2>
              <p className="mt-2 text-slate-400">Thanks for reaching out — the {COMPANY_NAME} team will get back to you at {form.email}.</p>
            </div>
          ) : (
            <form onSubmit={submit} className="space-y-4">
              <div className="grid gap-4 sm:grid-cols-2">
                <div><label className="block text-xs text-slate-400 mb-1.5">Your name</label><input value={form.name} onChange={(e) => set("name", e.target.value)} className={inp} placeholder="Jane Doe" /></div>
                <div><label className="block text-xs text-slate-400 mb-1.5">Email</label><input type="email" value={form.email} onChange={(e) => set("email", e.target.value)} className={inp} placeholder="you@example.com" /></div>
              </div>
              <div className="grid gap-4 sm:grid-cols-2">
                <div><label className="block text-xs text-slate-400 mb-1.5">Subject</label><input value={form.subject} onChange={(e) => set("subject", e.target.value)} className={inp} placeholder="How can we help?" /></div>
              ...
              <div><label className="block text-xs text-slate-400 mb-1.5">Message</label><textarea rows={5} value={form.message} onChange={(e) => set("message", e.target.value)} className={`${inp} resize-none`} placeholder="Tell us what's going on…" /></div>
              {/* Honeypot */}
              ...
              <p className="text-center text-[11px] text-slate-500">We&apos;ll only use your email to reply. See our Privacy Policy.</p>
            </form>
          )}
        </div>
      </div>
