  ...
  return (
    <div className="space-y-4 animate-pulse">
      <div className="space-y-2">
        <div className="h-7 w-56 rounded-lg bg-(--app-surface-2)" />
        <div className="h-4 w-72 rounded bg-(--app-surface-2)/70" />
      </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        {Array.from({ length: 4 }).map((_, i) => (
          <div
            key={i}
            className="h-20 rounded-xl border border-(--app-line) bg-(--app-surface)"
          />
        ))}
      </div>
      ...
      {Array.from({ length: 3 }).map((_, i) => (
        ...
        >
          <div className="h-32 w-full rounded-lg bg-(--app-surface-2)/70" />
          <div className="h-4 w-2/3 rounded bg-(--app-surface-2)" />
          <div className="h-3 w-1/3 rounded bg-(--app-surface-2)/70" />
        </div>
      ))}
    </div>
  );
