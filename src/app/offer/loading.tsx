...
export default function Loading() {
  return (
    <div className="mx-auto max-w-4xl px-4 py-10 space-y-6 animate-pulse">
      <div className="h-56 w-full rounded-xl bg-(--app-surface-2)/70" />
      <div className="h-8 w-2/3 rounded-lg bg-(--app-surface-2)" />
      <div className="space-y-2">
        {Array.from({ length: 5 }).map((_, i) => (
          <div key={i} className="h-4 w-full rounded bg-(--app-surface-2)/60" />
        ))}
      </div>
      <div className="h-12 w-48 rounded-lg bg-(--app-surface-2)" />
    </div>
  );
}
